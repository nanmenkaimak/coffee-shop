...
import (
	...
	"github.com/nanmenkaimak/product-images/files"
	"github.com/nanmenkaimak/product-images/handlers"
...
func main() {
	...
	s := http.Server{
		...
		IdleTimeout:  120 * time.Second, // max time for the connections using TCP Keep-Alive
	}

	// start the server
	go func() {
		l.Info("Starting server", "bind_adress", *bindAddress)
